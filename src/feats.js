async function bansheeCry(message) {
    ...
    if (!message.item.components.verbal) {
        return
    }
    ...
    if (messageType(message, 'attack-roll') && hasOption(message, "ranged")) {
        if (hasReaction(message?.target?.token?.combatant)) {
            ...
            if (ectoplasmic_shield) {
                postInChatTemplate(_uuid(ectoplasmic_shield), message.target?.token.combatant);
            }
        }
    }
}
        ...
        if (criticalSuccessMessageOutcome(message)) {
            if (hasReaction(message?.target?.token?.combatant)) {
                ...
                if (canReachEnemy(message.token, message?.target?.token, message?.target?.actor) && fv) {
                    postInChatTemplate(_uuid(fv), message.target?.token.combatant);
                }
            }
        }
    }
...
function stormRetribution(message) {
    if (messageType(message, 'attack-roll')) {
        if (criticalSuccessMessageOutcome(message)) {
            if (adjacentEnemy(message.token, message?.target?.token, message?.target?.actor)) {
                if (message?.item?.isMelee || message?.item?.traits?.has("unarmed")) {
                    if (message?.target?.actor?.system?.resources?.focus?.value > 0) {
                        ...
                        if (sr) {
                            postInChatTemplate(_uuid(sr), message.target?.token.combatant);
                        }
                    }
                }
            }
...
function courageousOpportunity(message) {
    if (isActorCharacter(message.actor)) {
        return
    }
    if (
...
function attackOfOpportunity(message) {
    if (
        (messageType(message, 'attack-roll') && message?.flags?.pf2e?.context?.domains.includes("ranged-attack-roll"))
...
function reactiveStrike(message) {
    if (
        (messageType(message, 'attack-roll') && message?.flags?.pf2e?.context?.domains.includes("ranged-attack-roll"))
        || messageWithAnyTrait(message, ["manipulate", "move"])
    ) {
...
function verdistantDefense(message) {
    if (!isActorCharacter(message.actor) && messageWithTrait(message, "move")) {
        characterWithReaction()
...
function eerieFlicker(message) {
    if (messageType(message, "attack-roll") && anySuccessMessageOutcome(message)) {
        if (hasReaction(message?.target?.token?.combatant)) {
            let eerieFlicker = actorFeat(message?.target?.actor, "eerie-flicker");
            if (eerieFlicker
                && !hasCondition(message?.target?.actor, "concealed")
                && !hasCondition(message?.target?.actor, "hidden")
                && !hasCondition(message?.target?.actor, "undetected")
            ) {
                postInChatTemplate(_uuid(eerieFlicker), message.target.token.combatant);
            }
        }
    }
}
...
async function scarsOfSteel(message) {
    ...
    if (game.pf2e.Predicate.test([
        ...
        if (scarsOfSteel) {
            postInChatTemplate(_uuid(scarsOfSteel), message.target.token.combatant);
        }
    }
}
